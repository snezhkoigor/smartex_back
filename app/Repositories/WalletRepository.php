...
class WalletRepository
{
	 ...
	 * @return int
	 */
	public static function getWalletCount(PaymentSystem $payment_system): int
	{
	 ...
	 * @return array
	 */
	public static function getAvailableWallets(array $filters = []): array
	{
		$result = [];
	 ...
	 * @return array
	 */
	public static function getAvailableWalletsForCommission(array $filters = []): array
	{
		$result = [];
		$query = Wallet::query();
		self::applyFiltersToQuery($query, $filters);
		...
		foreach ($wallets as $wallet) {
			$result[$wallet['id']] = [
				...
				'id' => $wallet['id'],
				'label' => $wallet->paymentSystem['name'] . ', ' . CurrencyRepository::getAvailableCurrencies()[$wallet['currency']]['prefix'] . ' (' . StringHelper::truncate($wallet['account'], 25) . ')',
				'value' => (int)$wallet['id']
			];
		}
	 ...
	 * @return Builder
	 */
	public static function getWalletQuery(PaymentSystem $payment_system): Builder
	{
		$query = Wallet::query();
